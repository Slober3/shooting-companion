...
## Pipeline (`dual-zone-evidence-v2`)
...
2. Estimate the central dark target zone from a strongly smoothed image. Light
   ...
   - light area: dark-core / black-hat response;
   - dark area: bright torn-fibre / top-hat response, optionally supported by a
     darker core.
...
Aggregate suppression counters are emitted in `diagnosticMetrics`.
