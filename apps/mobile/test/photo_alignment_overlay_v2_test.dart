...
void main() {
  ...
  testWidgets('four-point fine tuning changes authoritative corners', (
    tester,
  ) async {
    const initial = geo.NormalizedQuad(
      topLeft: geo.NormalizedPoint(0.1, 0.1),
    ...
    await tester.tap(find.text('Overlay aanpassen'));
    await tester.pump();
    await tester.ensureVisible(find.byTooltip('Overlay naar rechts'));
    await tester.tap(find.byTooltip('Overlay naar rechts'));
    await tester.pump();
    await tester.ensureVisible(find.text('Uitlijning klopt'));
    await tester.tap(find.text('Uitlijning klopt'));
    await tester.pump();
    ...
    expect(confirmed, isNotNull);
    expect(confirmed!.corners.topLeft.x, closeTo(0.1025, 1e-9));
    expect(
      confirmed!.normalizedToPhysical(const geo.NormalizedPoint(0.5025, 0.5)),
      const geo.PhysicalPointMm(0, 0),
    );
  });

  testWidgets('dragging in overlay mode translates the complete overlay', (
    tester,
  ) async {
    const initial = geo.NormalizedQuad(
      topLeft: geo.NormalizedPoint(0.2, 0.2),
      topRight: geo.NormalizedPoint(0.8, 0.2),
      bottomRight: geo.NormalizedPoint(0.8, 0.8),
      bottomLeft: geo.NormalizedPoint(0.2, 0.8),
    );
    geo.ManualPhotoAlignment? confirmed;
    await tester.pumpWidget(
      MaterialApp(
    ...
    final gestureLayer = find.byKey(
      const ValueKey('four-point-overlay-adjustment-gesture'),
    );
    await tester.ensureVisible(gestureLayer);
    await tester.pump();
    await tester.drag(gestureLayer, const Offset(60, 0));
    await tester.pump();
    await tester.ensureVisible(find.text('Uitlijning klopt'));
    await tester.tap(find.text('Uitlijning klopt'));
    await tester.pump();
    ...
    expect(confirmed, isNotNull);
    expect(confirmed!.corners.topLeft.x, greaterThan(initial.topLeft.x));
    expect(confirmed!.corners.topLeft.y, closeTo(initial.topLeft.y, 1e-9));
  });
