...
void main() {
  ...
  test('analysis diagnostics survive a concept reload', () async {
    final image = await _storedImage(documents, 'diagnostics.jpg');
    final scanId = await repository.createDraft(image: image);
    await repository.saveAnalysisResult(
      scanId,
      _completedAnalysis(
        diagnosticMetrics: const {
          'candidate_count': 0,
          'candidate_raw_component_count': 12,
          'candidate_rejected_weak_evidence_count': 7,
        },
        warnings: const [
          VisionWarning(
            code: VisionWarningCode.noCandidateImpacts,
            message: 'Geen veilig treffer-voorstel gevonden.',
          ),
        ],
      ),
    );
    ...
    final envelope = (jsonDecode(draft.qualityJson!) as Map)
        .cast<String, Object?>();
    expect(
      (envelope['_diagnosticMetrics'] as Map)['candidate_raw_component_count'],
      12,
    );
    expect(
      ((envelope['_analysisWarnings'] as List).single as Map)['code'],
      VisionWarningCode.noCandidateImpacts.name,
    );
    expect(envelope['_visionBackend'], VisionBackend.openCv.name);
  });
}
