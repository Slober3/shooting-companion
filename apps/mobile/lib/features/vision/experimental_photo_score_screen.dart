...
class _VisionScanFlowScreenState extends ConsumerState<VisionScanFlowScreen> {
  late final VisionAnalyzer _analyzer;
  bool _running = false;
  ...
  void initState() {
    super.initState();
    _analyzer = ref.read(visionAnalyzerProvider);
  }

  @override
  void dispose() {
    _analysisGeneration++;
    unawaited(_analyzer.cancel(widget.scanId));
    super.dispose();
  }
    ...
    return Scaffold(
      ...
      body: draftAsync.when(
        data: (draft) {
          ...
          final qualityEnvelope = draft.qualityJson == null
              ? null
          ...
          return ListView(
            ...
            children: [
              ...
              if (draft.qualityJson != null) ...[
                const SizedBox(height: 16),
                _QualityPanel(
                  quality: VisionQualityAssessment.fromJson(qualityEnvelope!),
                  registration: _registration(draft),
                ),
                if (qualityEnvelope['_diagnosticMetrics']
                    case final Map metrics)
                  _DetectorDiagnosticsPanel(
                    metrics: metrics.cast<String, Object?>(),
                    warningItems:
                        (qualityEnvelope['_analysisWarnings'] as List?) ??
                        const [],
                  ),
              ],
              const SizedBox(height: 20),
  ...
  Future<void> _runAnalysis() async {
    ...
    final draft = await repository.getDraft(widget.scanId);
    if (!_isCurrentAnalysis(generation) || draft == null) return;
    setState(() => _running = true);
    try {
      final capabilities = await _analyzer.capabilities();
      ...
      if (_isCurrentAnalysis(generation)) {
      ...
      if (_isCurrentAnalysis(generation)) {
        ...
        if (!mounted || generation != _analysisGeneration) return;
        AppMessenger.error(context, 'Analyse mislukt: $error');
      }
    } finally {
      if (_isCurrentAnalysis(generation)) {
  ...
  bool _isCurrentAnalysis(int generation) =>
      mounted && generation == _analysisGeneration;

  Future<void> _cancelAnalysis() async {
    await _analyzer.cancel(widget.scanId);
  }
...
class _DetectorDiagnosticsPanel extends StatelessWidget {
  const _DetectorDiagnosticsPanel({
    required this.metrics,
    required this.warningItems,
  });

  final Map<String, Object?> metrics;
  final List<Object?> warningItems;

  double _metric(String key) => (metrics[key] as num?)?.toDouble() ?? 0;

  @override
  Widget build(BuildContext context) {
    final candidates = _metric('candidate_count').round();
    final structures = _metric('candidate_raw_component_count').round();
    final rejectedGeometry = _metric(
      'candidate_rejected_geometry_count',
    ).round();
    final rejectedPrint = _metric('candidate_rejected_print_count').round();
    final rejectedWeak = _metric(
      'candidate_rejected_weak_evidence_count',
    ).round();
    final warnings = warningItems
        .whereType<Map>()
    ...
    return Card(
      child: ExpansionTile(
        ...
        title: Text(
          candidates > 0
              ? '$candidates treffer-voorstellen'
              : 'Geen treffer voorgesteld',
        ),
        subtitle: Text(
          structures == 0
              ? 'Geen lokale gatstructuur gevonden'
              : '$structures beeldstructuren gecontroleerd',
        ),
        childrenPadding: const EdgeInsets.fromLTRB(16, 0, 16, 16),
        children: [
          const Text(
            'Dit is diagnostiek van de experimentele detector. Afwijzingen '
            'veranderen de foto of score niet; je kunt alle treffers nog '
            'handmatig plaatsen.',
          ),
          const SizedBox(height: 12),
          _DiagnosticRow(label: 'Geometrie afgewezen', value: rejectedGeometry),
          _DiagnosticRow(
            label: 'Drukwerk/ring afgewezen',
            value: rejectedPrint,
          ),
          _DiagnosticRow(label: 'Bewijs te zwak', value: rejectedWeak),
          _DiagnosticRow(
            label: 'Hoge zekerheid',
            value: _metric('candidate_high_confidence_count').round(),
          ),
          _DiagnosticRow(
            label: 'Middelmatige zekerheid',
            value: _metric('candidate_medium_confidence_count').round(),
          ),
          _DiagnosticRow(
            label: 'Lage zekerheid',
            value: _metric('candidate_low_confidence_count').round(),
          ),
